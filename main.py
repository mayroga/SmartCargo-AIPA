...
# Aquí es donde se conecta con tu variable de Render:
...
async def process_audit(cargo: CargoAudit):
    ...
    # Lógica de Seguridad y Soluciones
    if H_cm > 158:
...
async def advisory_vision(prompt: str = Form(...), image: Optional[UploadFile] = File(None)):
    ...
    parts = [{"text": f"Eres SMARTCARGO CONSULTING. Objetivo: Dar soluciones técnicas bilingües breves. Prompt: {prompt}"}]
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            answer = "Error: Verifica tu Gemini Key o el formato de imagen."
        return {"data": answer}

@app.post("/create-payment")
async def payment(amount: float = Form(...), awb: str = Form(...), password: Optional[str] = Form(None)):
    # BYPASS DE ADMIN: Comparamos lo que envías en el prompt con la variable de Render

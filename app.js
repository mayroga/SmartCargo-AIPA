...
function startTimer(mins, tier) {
    ...
    setInterval(() => {
        timeLeft--;
        idleTime++;
        if (idleTime === 59) { alert("⚠️ TOQUE LA PANTALLA para mantener la asesoría activa."); resetIdle(); }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (params.get("access") === "granted") {
        ...
        startTimer(params.get("tier") == "45" ? 20 : (params.get("tier") == "95" ? 45 : 8), params.get("tier"));
    }

    document.getElementById("activateBtn").onclick = async () => {
        if(!selectedRole || selectedAmount === 0) return alert("Seleccione datos.");
    ...
    document.getElementById("advForm").onsubmit = async (e) => {
        ...
        out.innerHTML = "<strong>⚙️ ASESOR ANALIZANDO...</strong>";
        const fd = new FormData();
        fd.append("prompt", `Rol: ${selectedRole}. Problema: ${document.getElementById("promptArea").value}`);
